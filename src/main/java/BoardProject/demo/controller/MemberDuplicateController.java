...
public class MemberDuplicateController {
    ...
    @PostMapping("/member/telValidation")
    public ResponseEntity<Map<String, Object>> validateTel(@RequestBody Map<String, String> requestData) {
        String tel = requestData.get("tel");
        Map<String, Object> response = new HashMap<>();
        boolean isValid = memberService.isExistMemberByTel(tel);
        response.put("isValid", isValid);
        return ResponseEntity.ok(response);
    }
}
